...
class SignalProcessWindowFunction(ProcessWindowFunction):
    ...
    def open(self, runtime_context: RuntimeContext):
        ...
        # 存储上一条记录
        prev_record_state_name = ['timestamp', 'utm_x', 'utm_y', 'speed', 'heading', 'driverlongitude',
                                  'driverlatitude']
        prev_record_state_type = [Types.LONG(), Types.DOUBLE(), Types.DOUBLE(), Types.DOUBLE(), Types.DOUBLE(),
                                  Types.DOUBLE(), Types.DOUBLE()]
        self.prev_record_state = runtime_context.get_state(
            ValueStateDescriptor("SignalProcess_prev_record_state",
                                 Types.ROW_NAMED(prev_record_state_name, prev_record_state_type))
        )
        # 记录相同的signal_id的记录最早的 ： latitude/longitude/height/drone_uuid/lasting_time/timestamp
        signal_first_state_name = ['latitude', 'longitude', 'height', 'drone_uuid', 'lasting_time', 'timestamp',
                                   'batch_num']
        signal_first_state_type = [Types.DOUBLE(), Types.DOUBLE(), Types.DOUBLE(), Types.STRING(), Types.INT(),
                                   Types.LONG(), Types.STRING()]
        self.signal_first_state = runtime_context.get_state(
            ValueStateDescriptor("SignalProcess_first_state",
                                 Types.ROW_NAMED(signal_first_state_name, signal_first_state_type))
        )
    ...
    def process(self, key, context, elements):
        ...
        for record in sorted_elements:
            ...
            if win_start_time <= record['timestamp'] * 1000 < win_start_time + self.slide:
            ...
            if signal_first is not None:
                ...
                if record['timestamp'] < signal_first['timestamp'] or record['lasting_time'] == 0:
                    # 更新数据
                    signal_first = Row(latitude=record['latitude'], longitude=record['longitude'],
                                       height=record['height'], drone_uuid=record['drone_uuid'],
                                       lasting_time=record['lasting_time'], timestamp=record['timestamp'],
                                       batch_num=record['batch_num'])
                    self.signal_first_state.update(signal_first)
                    # print(f"打印 signal_first: {signal_first}")
            else:
                signal_first_org = Row(latitude=record['latitude'], longitude=record['longitude'],
                                       height=record['height'], drone_uuid=record['drone_uuid'],
                                       lasting_time=record['lasting_time'], timestamp=record['timestamp'],
                                       batch_num=record['batch_num'])
...
class SimpleProcessWindowFunction(ProcessWindowFunction):
    def __init__(self):
        self.last_fusion_state = None

    def open(self, runtime_context: RuntimeContext):
        last_fusion_state_name = ['drone_uuid', 'batch_num', 'fusion_id', 'fusion_start_time', 'fusion_longitude',
                                  'fusion_latitude', 'fusion_height']
        last_fusion_state_type = [Types.STRING(), Types.STRING(), Types.STRING(), Types.STRING(), Types.DOUBLE(),
                                  Types.DOUBLE(), Types.DOUBLE()]
        # 存储上一条记录的fusion相关信息
        self.last_fusion_state = runtime_context.get_map_state(
            MapStateDescriptor("SimpleProcess_last_fusion_state", Types.STRING(),
                               Types.ROW_NAMED(last_fusion_state_name, last_fusion_state_type))
        )
    ...
    def process(self, context, elements):
        ...
        for cr in elements:
            # print(f"cr: {cr}")
            pre_key = cr.get('drone_uuid') + '_' + cr.get('batch_num') if cr.get('drone_uuid') and cr.get(
                'batch_num') else None
            ...
            for sid in signal_ids:
                if sid in signal_id_to_records:
                    for rec in signal_id_to_records[sid]:
                        ...
                        map_vlu = Row(drone_uuid=rec['drone_uuid'], batch_num=rec['batch_num'],
                                      fusion_id=rec['fusion_id'],
                                      fusion_start_time=rec['fusion_start_time'],
                                      fusion_longitude=rec['fusion_longitude'],
